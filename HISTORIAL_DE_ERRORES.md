...
|---|---|---|---|---|---|
| 20/04/2025 | Producción | **CRÍTICO:** Todos los datos (productos, categorías) devuelven `401 Unauthorized`. Sitio muestra solo datos estáticos de Enero. | La variable `VITE_SUPABASE_ANON_KEY` configurada en Vercel usaba el formato NUEVO `sb_publishable_...` que **no es compatible** con la API REST de Supabase (PostgREST). Solo funciona la clave JWT clásica que empieza con `eyJ...` | 1. Ir a Supabase → Project Settings → API → copiar la clave `anon public` (formato JWT `eyJ...`). 2. Actualizar `VITE_SUPABASE_ANON_KEY` en Vercel con esa clave. 3. Redeploy. **NUNCA usar el formato `sb_publishable_` como anon key en Vercel.** | ✅ Resuelto |
| 20/04/2025 | Producción | Sidebar muestra solo "Todas" — sin categorías ni subcategorías | La tabla `categorias` en Supabase tenía RLS activado pero sin política de lectura pública. El cliente anon no podía hacer SELECT. | Ejecutar en SQL Editor: `ALTER TABLE categorias ENABLE ROW LEVEL SECURITY; CREATE POLICY "Public read categorias" ON categorias FOR SELECT TO anon, authenticated USING (true);` | ✅ Resuelto |
| 20/04/2025 | Producción | Site caído por completo (Error DNS) tras configurar registros Zoho Mail | Se modificaron registros DNS en Cloudflare que interrumpieron el proxy hacia el hosting. La zona quedó sin destino válido. | 1. Restaurar el registro A/CNAME de `fycferreteria.cl` apuntando a Vercel. 2. Proxy Cloudflare: ON (naranja). 3. HTTP/3: OFF para evitar conflictos con Vercel. | ✅ Resuelto |
| 18/04/2025 | Producción | AdminDashboard guarda cambios pero no persisten en Supabase (fallas silenciosas por RLS/cache) | 1. RLS bloqueaba `anon_key` en los `.update()`. 2. Las actualizaciones optimistas encubrían el fallo real de la DB y `fetchProductos()` retrocedía a datos stales de caché. Error por `.eq('sku')` ineficiente. | Implementado `adminSupabase` con `VITE_SUPABASE_SERVICE_ROLE_KEY` bypass estricto. UI de guardado lee error real con log explícito. `.trim()` estricto al SKU original (`targetSku`). Eliminada la actualización "optimista". | ✅ Resuelto |
