...
interface SidebarProps {
  ...
  absMaxPrice: number;
  allProducts?: any[]; // Para contadores dinámicos por categoría
}
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  // Contador dinámico: cantidad de productos visibles por categoría
  ...
  return (
    <aside className="w-full lg:w-72 flex-shrink-0 space-y-6">
      ...
      <div className="bg-[#0E0E0E] border border-[#222] p-6 rounded-3xl shadow-xl">
        ...
        <ul className="space-y-1.5 max-h-[65vh] overflow-y-auto pr-2 custom-scrollbar">
          {CATEGORIES.map((cat) => {
            ...
            return (
              <li key={cat}>
                <button
                  ...
                  className={`w-full text-left px-4 py-2.5 text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-between group rounded-full ${isActive
                      ? 'text-black bg-[#FFD700] shadow-lg transform scale-105'
                      : count === 0
                        ? 'text-neutral-700 hover:text-neutral-400 hover:bg-[#111] border border-transparent'
                        : 'text-neutral-400 hover:text-white hover:bg-[#151515] border border-transparent'
                    }`}
                >
                  <span>{cat}</span>
                  <span className={`text-[10px] font-black ml-2 px-2 py-0.5 rounded-full flex-shrink-0 ${isActive
